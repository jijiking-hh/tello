...
class guidance:
    def __init__(self):
        ...
        rospy.Subscriber('/tello2/ground_truth_to_tf/pose', PoseStamped, self.uav2_pose_callback)
        rospy.Subscriber('/tello3/ground_truth_to_tf/pose', PoseStamped, self.uav3_pose_callback)
        rospy.Subscriber('/tello4/ground_truth_to_tf/pose', PoseStamped, self.target_pose_callback)
        ...
        while not rospy.is_shutdown():
            ...
            if command == 1:
                r1, q_lon1, q_lat1, thet1, psi1, uav1_vel = self.bili_guidance(uav1_pos, target_pos, r1, q_lon1, q_lat1,thet1, psi1)
                # print(q_lon1, q_lat1, thet1, psi1)
                print(uav3_vel)
                r2, q_lon2, q_lat2, thet2, psi2, uav2_vel = self.bili_guidance(uav2_pos, target_pos, r2, q_lon2, q_lat2,thet2, psi2)
                # print(q_lon2, q_lat2, thet2, psi2)
                r3, q_lon3, q_lat3, thet3, psi3, uav3_vel = self.bili_guidance(uav3_pos, target_pos, r3, q_lon3, q_lat3,thet3, psi3)
		target_vel.linear.x=0.01
		target_vel.linear.y=0.03	
                uav1_vel_pub.publish(uav1_vel)
                uav2_vel_pub.publish(uav2_vel)
                uav3_vel_pub.publish(uav3_vel)
                target_vel_pub.publish(target_vel)
                if r1 < 0.0002 or r2 < 0.0002 or r3 < 0.0002:
                    command = 0
                    # print(r1, r2, r3)
                    print('Bomb!!!')
            else:
